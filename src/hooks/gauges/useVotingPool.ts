...
import {
    ...
    useVoterGetCurrentPeriod,
    useVoterGetGauge,
    useVotingRewardGetRewardList,
    useVotingRewardTotalVotesInPeriod,
    votingRewardABI,
} from "@/generated";
...
export function useVotingPool(poolAddress: Address | undefined) {
    ...
    const { data: gauge, refetch: refetchGauge } = useVoterGetGauge({
        args: [poolAddress!],
        enabled: !!poolAddress,
    });

    const { data: currentPeriod, isLoading: currentPeriodLoading } = useVoterGetCurrentPeriod();
    ...
    const { data: token0Address, isLoading: isLoadingToken0 } = useAlgebraPoolToken0({
        address: poolAddress,
    });
    const { data: token1Address, isLoading: isLoadingToken1 } = useAlgebraPoolToken1({
        address: poolAddress,
    });
