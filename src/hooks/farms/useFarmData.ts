...
export function useFarmData(farm: FarmingFieldsFragment | null | undefined) {
  ...
  const { data: isDynamicRateActivated } =
    useAlgebraVirtualPoolDynamicRateActivated({
      address: virtualPool,
    });

  const formattedReward = _rewardToken
    ? Number(formatUnits(BigInt(reward), _rewardToken.decimals)).toFixed(3)
    : undefined;
  const formattedBonusReward = _bonusRewardToken
    ? Number(
        formatUnits(BigInt(bonusReward), _bonusRewardToken.decimals)
      ).toFixed(3)
    : undefined;

  const formattedRewardRate = _rewardToken
    ? Number(formatUnits(BigInt(rewardRate), _rewardToken.decimals))
    : undefined;
