...
const AdminAccount = () => {
    ...
    const { data: isAdmin, isLoading: isLoadingAdmin } = useAlgebraFactoryHasRoleOrOwner({
        args: account ? [ADMIN_ROLE_CONSTANT, account] : undefined,
    });

    const { data: isPoolAdmin, isLoading: isLoadingPoolAdmin } = useAlgebraFactoryHasRoleOrOwner({
