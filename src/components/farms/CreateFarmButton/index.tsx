...
const CreateFarmButton = ({
  ...
  const isKeyReady =
    ...
    plugin &&
    nonce !== undefined &&
    (hasSecondReward ? Boolean(bonusRewardToken) : true);

  const areRewardsReady =
    rewardBn &&
    rewardRateBn &&
    (hasSecondReward ? Boolean(bonusRewardBn && bonusRewardRateBn) : true);
  ...
  const {
    approvalState: approvalStateReward,
    approvalCallback: approvalCallbackReward,
  } = useApprove(parsedRewardAmount, ALGEBRA_ETERNAL_FARMING);
  const {
    approvalState: approvalStateBonusReward,
    approvalCallback: approvalCallbackBonusReward,
  } = useApprove(parsedBonusRewardAmount, ALGEBRA_ETERNAL_FARMING);

  const showApproveReward =
    approvalStateReward === ApprovalState.NOT_APPROVED ||
    approvalStateReward === ApprovalState.PENDING;
  const showApproveBonusReward =
    approvalStateBonusReward === ApprovalState.NOT_APPROVED ||
  ...
  if (showApproveBonusReward)
    return (
      <button
        ...
        onClick={() =>
          approvalCallbackBonusReward && approvalCallbackBonusReward()
        }
        className="flex justify-center py-4 bg-blue-500 text-white font-bold rounded-xl hover:bg-blue-600 disabled:bg-blue-300 disabled:cursor-not-allowed"
      >
        {approvalStateBonusReward === ApprovalState.PENDING ? (
